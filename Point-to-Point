...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    MPI_Recv(&local_value, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    // 3- Print the sum of the two values on stdout 
    std::cout << "Processo #1: %d" << local_value << std::endl;
  }
  else {
    ...
    std::cout << "Processo #1: %d" << local_value * other_value << std::endl;
  }
